...
from annie.npc.observability.tracing import EventType
...
class Executor:
    ...
    def __call__(self, state: AgentState) -> dict:
        ...
        with span:
            ...
            for task in tasks:
                task, result, messages = self._execute_task(
                    ctx,
                    task,
                    working_memory,
                    todo_text,
    ...
    def _execute_task(
    ...
    ) -> tuple[Task, dict[str, Any] | None, list[BaseMessage]]:
        ...
        content = self._classify_task_outcome(task, loop_result)
        result = self._build_task_result(
            task,
            content,
            runtime,
            action_results_baseline,
        )
        return task, result, messages

    def _classify_task_outcome(self, task: Task, loop_result: LoopResult) -> str:
...
def _has_runtime_productive_effect(runtime: dict[str, Any], baseline: dict[str, int]) -> bool:
    return any(
        ...
        for key in ("action_results", "memory_updates")
    )
