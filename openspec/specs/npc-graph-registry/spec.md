# npc-graph-registry Specification

## Purpose
TBD - created by archiving change introduce-npc-graph-registry. Update Purpose after archive.
## Requirements
### Requirement: NPC Agent exposes graph-id dispatch

The NPC Agent layer SHALL expose a first-class `route_id` dispatch contract on `AgentContext` for selecting a pre-registered cognitive route.

#### Scenario: Explicit route selection

- **WHEN** a world engine constructs an `AgentContext` with a registered `route_id`
- **THEN** `NPCAgent.run()` executes the graph registered for that identifier
- **AND** route selection does not depend on parsing `input_event`, tool names, or business metadata

#### Scenario: Unknown graph is rejected

- **WHEN** a world engine requests a `route_id` that is not registered by the NPC layer
- **THEN** `NPCAgent.run()` fails with a clear route selection error
- **AND** it does not fall back to a different graph silently

#### Scenario: Default route selection

- **WHEN** a world engine omits `route_id`, `route`, and legacy direct-mode flags
...
The NPC Agent layer SHALL own the registry of legal cognitive routes and their construction.

#### Scenario: Registered routes are built inside NPC layer
...
- **WHEN** the NPC layer registers an initial cognitive route
- **THEN** the registry entry contains a stable identifier, route builder, and minimal policy metadata
- **AND** it does not require a new declarative graph DSL for nodes and edges

### Requirement: Route identifiers are business-agnostic

Registered route identifiers SHALL describe generic cognitive or output flow and SHALL NOT encode concrete world-engine business domains.

#### Scenario: Generic graph names

- **WHEN** the NPC layer defines route identifiers for action, dialogue, structured output, or reflection
...
- **THEN** it selects a generic route identifier
- **AND** it provides the business-specific prompt text, evidence, tools, or output requirements through `AgentContext`

### Requirement: Route compatibility maps to route identifiers

The NPC Agent layer SHALL provide temporary compatibility mappings from existing route-based dispatch inputs to default route identifiers.

#### Scenario: Route maps to graph

- **WHEN** a world engine provides `AgentContext.route` but omits `route_id`
- **THEN** `NPCAgent.run()` maps that route to the corresponding default route identifier
- **AND** it executes the mapped graph through the route registry

#### Scenario: Graph id takes precedence

- **WHEN** a world engine provides both `route_id` and `route`
- **THEN** `NPCAgent.run()` selects the explicit `route_id`
- **AND** debug metadata records the selected route identifier and route-kind classification

#### Scenario: Legacy direct mode maps to graph

- **WHEN** an existing integration still provides legacy `extra["npc_direct_mode"]`
- **THEN** `NPCAgent.run()` maps the direct-mode value to a default route identifier during the migration period
- **AND** the mapped execution follows the same tool and output policy as explicit route selection

### Requirement: Graph policy controls tool availability

Each registered route SHALL carry or derive policy metadata sufficient to enforce tool availability before LLM invocation and tool dispatch.
...
The NPC Agent layer SHALL expose bounded observability for route execution without making full internal traces part of the stable public API.

#### Scenario: Response reports selected graph

- **WHEN** any graph completes
- **THEN** `AgentResponse` or its debug metadata reports the selected route identifier
...
- **THEN** it includes bounded information such as route identifier, route-kind classification, node path, and bound tool names
...
The NPC Agent layer SHALL keep per-run and per-NPC data out of the route registry and compiled graph cache.

#### Scenario: Per-run data enters through context

- **WHEN** the same registered route is used for multiple NPCs or multiple runs
- **THEN** NPC identity, memory interface, tools, prompt text, evidence, and input event come from the current `AgentContext`
- **AND** no previous run's business state is read from the route registry
...
The NPC Agent route registry SHALL register each graph with enough metadata to describe its generic node composition and policy classification.

#### Scenario: Registry entry includes composition metadata

- **WHEN** the NPC layer registers a graph
- **THEN** the registry entry includes a stable route identifier, route or policy kind, route builder, response kind, and bounded node composition labels
...
- **WHEN** `NPCAgent.run()` resolves a registered route
- **THEN** the route builder and node wiring come from the NPC layer registry
