"""Tests for Planner node."""

import json
from unittest.mock import MagicMock

import pytest

from annie.npc.planner import Planner
from annie.npc.state import AgentState, NPCProfile, Personality, Goals, Task
from annie.npc.tracing import EventType, Tracer


@pytest.fixture
def mock_llm():
    llm = MagicMock()
    response = MagicMock()
    response.content = json.dumps([
        {"description": "Observe the stranger carefully", "priority": 8},
        {"description": "Recall past encounters with travelers", "priority": 6},
    ])
    llm.invoke.return_value = response
    return llm


@pytest.fixture
def npc_profile():
    return NPCProfile(
        name="Elder",
        personality=Personality(traits=["wise", "cautious"], values=["safety"]),
        goals=Goals(short_term=["protect the village"], long_term=["maintain peace"]),
    )


@pytest.fixture
def planner(mock_llm):
    return Planner(mock_llm)


class TestPlanner:
    def test_returns_tasks(self, planner, npc_profile):
        tracer = Tracer("Elder")
        state: AgentState = {
            "npc_profile": npc_profile,
            "input_event": "A stranger approaches the village.",
            "tracer": tracer,
        }
        result = planner(state)
        assert "tasks" in result
        assert len(result["tasks"]) == 2
        assert all(isinstance(t, Task) for t in result["tasks"])

    def test_task_descriptions(self, planner, npc_profile):
        tracer = Tracer("Elder")
        state: AgentState = {
            "npc_profile": npc_profile,
            "input_event": "A stranger approaches.",
            "tracer": tracer,
        }
        result = planner(state)
        descriptions = [t.description for t in result["tasks"]]
        assert "Observe the stranger carefully" in descriptions

    def test_task_priorities(self, planner, npc_profile):
        tracer = Tracer("Elder")
        state: AgentState = {
            "npc_profile": npc_profile,
            "input_event": "A stranger approaches.",
            "tracer": tracer,
        }
        result = planner(state)
        assert result["tasks"][0].priority == 8
        assert result["tasks"][1].priority == 6

    def test_tracing_events(self, planner, npc_profile):
        tracer = Tracer("Elder")
        state: AgentState = {
            "npc_profile": npc_profile,
            "input_event": "A stranger approaches.",
            "tracer": tracer,
        }
        planner(state)
        event_types = [e.event_type for e in tracer.events]
        assert EventType.NODE_ENTER in event_types
        assert EventType.NODE_EXIT in event_types
        assert EventType.LLM_CALL in event_types
        assert EventType.LLM_RESPONSE in event_types
        assert EventType.TASK_CREATED in event_types

    def test_llm_called_with_messages(self, planner, npc_profile, mock_llm):
        tracer = Tracer("Elder")
        state: AgentState = {
            "npc_profile": npc_profile,
            "input_event": "A stranger approaches.",
            "tracer": tracer,
        }
        planner(state)
        mock_llm.invoke.assert_called_once()
        messages = mock_llm.invoke.call_args[0][0]
        assert len(messages) == 2  # system + human

    def test_handles_markdown_code_block(self, npc_profile):
        llm = MagicMock()
        response = MagicMock()
        response.content = '```json\n[{"description": "Do something", "priority": 5}]\n```'
        llm.invoke.return_value = response
        planner = Planner(llm)
        tracer = Tracer("Elder")
        state: AgentState = {
            "npc_profile": npc_profile,
            "input_event": "Event",
            "tracer": tracer,
        }
        result = planner(state)
        assert len(result["tasks"]) == 1
        assert result["tasks"][0].description == "Do something"

    def test_handles_invalid_json(self, npc_profile):
        llm = MagicMock()
        response = MagicMock()
        response.content = "I think the NPC should do something"
        llm.invoke.return_value = response
        planner = Planner(llm)
        tracer = Tracer("Elder")
        state: AgentState = {
            "npc_profile": npc_profile,
            "input_event": "Event",
            "tracer": tracer,
        }
        result = planner(state)
        assert len(result["tasks"]) == 1  # Falls back to raw text as single task

    def test_works_without_tracer(self, planner, npc_profile):
        state: AgentState = {
            "npc_profile": npc_profile,
            "input_event": "A stranger approaches.",
        }
        result = planner(state)
        assert len(result["tasks"]) == 2

    def test_includes_memory_context(self, planner, npc_profile, mock_llm):
        tracer = Tracer("Elder")
        state: AgentState = {
            "npc_profile": npc_profile,
            "input_event": "A stranger approaches.",
            "memory_context": "Previously encountered bandits.",
            "tracer": tracer,
        }
        planner(state)
        messages = mock_llm.invoke.call_args[0][0]
        assert "bandits" in messages[1].content
