...
- [x] 0.1 从当前分支 `refactor/context-mgmt-and-agentic-rag` 新建 `refactor/memory-grep-and-prompt-overhaul`
- [x] 0.2 确认 `chromadb` 版本支持 `collection.get(where=...)` 的 metadata 过滤

## 阶段 1：MemoryInterface.grep

- [x] 1.1 `src/annie/npc/memory/interface.py`：Protocol 新增 `grep(pattern, category=None, metadata_filters=None, k=20) -> list[MemoryRecord]`
- [x] 1.2 `src/annie/world_engine/store.py`：新增 `grep_entries(pattern, where, k)` 方法（ChromaDB `get(where=...)` + Python 端 `casefold()` 子串匹配）
- [x] 1.3 `src/annie/world_engine/memory.py` (`DefaultMemoryInterface`)：实现 `grep`，`relevance_score` 固定 1.0，按 `created_at` 新→旧排序
- [x] 1.4 单元测试：
  - 基本子串命中
  - category 过滤
  - metadata_filters 叠加
...
- [x] 2.1 `src/annie/npc/tools/builtin.py`：新增 `MemoryGrepInput` + `MemoryGrepTool`
- [x] 2.2 `default_builtin_tools()` 加入 `MemoryGrepTool()`
- [x] 2.3 单元测试：tool 的 input/output 往返

## 阶段 3：Prompt 结构改造

- [x] 3.1 新建 `src/annie/npc/prompts.py`，提供 `render_identity(ctx) -> str` helper
- [x] 3.2 `src/annie/npc/executor.py`：
...
- [x] 3.3 `src/annie/npc/planner.py`：
...
- [x] 3.4 `src/annie/npc/state.py`：
  - 字段 `memory_context` → `working_memory`
  - 新增 `last_tasks: list[Task]`
- [x] 3.5 `src/annie/npc/agent.py`：
...
- [x] 4.1 `src/annie/npc/reflector.py`：
...
- [x] 5.1 `src/annie/npc/tools/builtin.py`：`InnerMonologueTool.call` 写 `ctx.agent_context.extra.setdefault("_inner_thoughts", []).append(...)`
- [x] 5.2 `_build_response` 从 `extra["_inner_thoughts"]` 读取拼装（阶段 3.5 已列，此处留空确认口径一致）

## 阶段 6：测试

- [x] 6.1 新增 `tests/test_world_engine/test_memory_grep.py` —— 覆盖 1.4 各场景
- [x] 6.2 新增 `tests/test_npc/test_prompts.py` —— `render_identity`、`<memory_categories>` / `<working_memory>` 段渲染
- [x] 6.3 更新 `tests/test_integration/test_decoupled_flow.py`：
  - state 字段改名适配
...
- [x] 6.4 新增 `tests/test_npc/test_reflector_parser.py`：JSON / bullet / 混合 / 空 四组
- [x] 6.5 新增 `tests/test_npc/test_retry_context.py`：retry_count=1 时 Planner user_content 含 `<retry_context>` 与上轮 task 描述

## 阶段 7：文档与收尾

- [x] 7.1 更新 `CLAUDE.md` 的 Architecture / 规约段落：
  - 双索引（recall + grep）
  - `working_memory` 命名
