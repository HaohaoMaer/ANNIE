...
- `tools/builtin.py` — built-in tools always registered: `memory_recall`, `memory_grep`, `memory_store`, `inner_monologue`. `inner_monologue` appends to `AgentContext.extra["_inner_thoughts"]`, which `_build_response` folds into `AgentResponse.inner_thought`.
- `prompts.py` — shared prompt fragments: `render_identity(ctx)` (used by both Executor & Reflector) and `MEMORY_CATEGORIES_BLOCK` (the five-line memory category catalog embedded in the Executor system prompt).
...
- `executor.py` — native **tool-use loop**. System prompt uses fixed XML sections in order: `<character>`, `<world_rules>`, `<situation>`, `<memory_categories>`, `<working_memory>`, `<available_skills>`. `<working_memory>` is Planner-stage pre-retrieved `memory_agent.build_context(input_event)` transparently passed through — Executor never triggers a second pre-retrieve. Tool names are **not** hardcoded in the system text; the model discovers them via the bound tool schema only. History enters as a separate message sequence. When Planner skips, `_executor_with_skip` synthesizes a `Task(description="__skip__")` and the trigger renders only `<input_event>` (no `<task>` section) to avoid redundancy. Loop runs `llm.bind_tools(all_tools).invoke(messages)` until no tool_calls (or `MAX_TOOL_LOOPS=8`). Dispatches tool_calls via `ToolAgent.dispatch` which **Micro-compresses** outputs >2000 chars.
...
- `memory.py` — `DefaultMemoryInterface`: single per-NPC ChromaDB collection (`npc_memory_{npc_id}`) with `category` metadata. `recall` (RAG) gives `impression` hits a 1.2× relevance boost; `grep` does substring-match via `collection.get(where=...)` + Python-side `casefold()` and returns results newest-first (no similarity weighting).
