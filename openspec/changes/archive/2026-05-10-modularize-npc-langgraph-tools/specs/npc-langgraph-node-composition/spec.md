...
- **WHEN** the NPC layer defines nodes for registered graphs
...
- **THEN** it may select a registered `graph_id`
...
### Requirement: NPC nodes use a shared per-run graph state

NPC graph nodes SHALL exchange per-run information through an NPC-owned graph state object derived from the current `AgentContext`.

#### Scenario: Per-run data enters through graph state

- **WHEN** a registered graph runs for an NPC
- **THEN** the initial graph state is built from the current `AgentContext`
...
### Requirement: Output nodes project graph state into AgentResponse

NPC output nodes SHALL convert graph state into stable `AgentResponse` fields without performing world-engine business validation or persistence.
...
NPC graph execution SHALL expose bounded node-level observability for tests and debugging without making full LangGraph traces part of the stable API.

#### Scenario: Node path is reported

- **WHEN** a registered graph completes
- **THEN** `AgentResponse` debug metadata includes the selected graph identifier and bounded node path labels
...
- **WHEN** graph debug metadata is produced
